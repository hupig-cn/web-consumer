package com.weisen.www.code.yjf.consumer.security.oauth2;

import com.weisen.www.code.yjf.consumer.config.oauth2.OAuth2Properties;
import com.weisen.www.code.yjf.consumer.web.filter.RefreshTokenFilter;
...
public class OAuth2AuthenticationServiceTest {
    ...
    public void testRefreshGrant() {
        ...
        Assertions.assertEquals(NEW_ACCESS_TOKEN_VALUE, newAccessTokenCookie.getValue());
        Cookie newRefreshTokenCookie = response.getCookie(OAuth2CookieHelper.REFRESH_TOKEN_COOKIE);
        Assertions.assertEquals(NEW_REFRESH_TOKEN_VALUE, newRefreshTokenCookie.getValue());
        Cookie requestAccessTokenCookie = OAuth2CookieHelper.getAccessTokenCookie(newRequest);
        Assertions.assertEquals(NEW_ACCESS_TOKEN_VALUE, requestAccessTokenCookie.getValue());
    }
    ...
    public void testRefreshGrantNoRefreshToken() {
        ...
        MockHttpServletResponse response = new MockHttpServletResponse();
        Assertions.assertThrows(InvalidTokenException.class, () -> refreshTokenFilter.refreshTokensIfExpiring(request, response));
    }
